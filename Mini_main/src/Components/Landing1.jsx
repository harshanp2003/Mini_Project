...
import social_img from './Assets/all_image.jpg';
    ...
    <header class="shadow mb-2">
      <div class="relative flex max-w-screen-xl flex-col px-4 py-4 md:mx-auto md:flex-row md:items-center">
        <a href="#" class="flex items-center whitespace-nowrap text-2xl font-black">
          ...
          <img src={logo_img} alt="" className='logo_img' style={{height:'1em',width:'1em'}}/>
        ...
        <nav aria-label="Header Navigation" class="peer-checked:mt-8 peer-checked:max-h-56 flex max-h-0 w-full flex-col items-center justify-between overflow-hidden transition-all md:ml-24 md:max-h-full md:flex-row md:items-start">
          <ul class="flex flex-col items-center space-y-2 md:ml-auto md:flex-row md:space-y-0">
            <li class="text-gray-600 md:mr-12 hover:text-blue-600"><a href="#about">About</a></li>
            {/* <li class="text-gray-600 md:mr-12 hover:text-blue-600"><a href="#">Features</a></li> */}
            <li class="text-gray-600 md:mr-12 hover:text-blue-600"><a href="#Contact">Contact us</a></li>
            <li class="text-gray-600 md:mr-12 hover:text-blue-600">
              <button class="rounded-md border-2 border-blue-600 px-6 py-1 font-medium text-blue-600 transition-colors hover:bg-blue-600 hover:text-white" onClick={() => nav("/register")}>Login</button>
            </li>
          </ul>
        </nav>
    </div>
        ...
        {/* <div className="btn-group">
          <button className="btn btn-sm btn-primary dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">
              Filter
          </button>
          <ul className="dropdown-menu">

            <li><button class="dropdown-item" onClick={()=>nav('/user_issues/status',{state:{userid:data.u_id,selected:'solved'}})}>Solved</button></li>
            <li><button class="dropdown-item" onClick={()=>nav('/user_issues/status',{state:{userid:data.u_id,selected:'pending'}})}>Pending</button></li>
            <li><button class="dropdown-item" onClick={()=>nav('/user_issues/status',{state:{userid:data.u_id,selected:'rejected'}})}>Rejected</button></li>
          </ul>
        </div>
      </div>
      <li class="md:mr-12">
        <button type='submit' class="rounded-full border-2 border-blue-900 px-6 py-1 text-blue-900 transition-colors hover:bg-blue-900 hover:text-white" onClick={() => nav('/')}>Logout</button>
      </li> */}
    </header>
   <div>
         <section>
      ...
      {/* Line 72 is to visulaize the working of admin jsx */}

       {/* <button type='submit' onClick={()=>nav("/admin/access")}>click here to view main form</button> */}
</div>  
       )
  }
